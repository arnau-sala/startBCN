...
export function LocalTrendsSheet({ open, onClose, triggerRef }: LocalTrendsSheetProps) {
    ...
    return (
        <>
            {/* Backdrop — fade in */}
            <div
                className="fixed inset-0 z-50 lt-backdrop-enter"
                style={{ background: "rgba(15,23,42,0.5)" }}
                onClick={onClose}
                aria-hidden="true"
            />

            {/* Modal — appears directly (no slide) */}
            <div
                ref={sheetRef}
                role="dialog"
                aria-modal="true"
                aria-label="Local trends map"
                className="fixed left-1/2 top-1/2 z-50 flex w-[98%] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden lt-sheet-enter"
                style={{
                    maxWidth: "1200px",
                    height: "92vh",
            ...
            >
                ...
                <div
                    className="flex shrink-0 items-center justify-between px-4 pb-3 pt-3"
                ...
                {/* Footer hint */}
                <div
                    className="shrink-0 flex flex-col items-center justify-center gap-1 px-4 py-3"
            ...
            <style>{`
        @keyframes lt-backdrop-fade {
          from { opacity: 0; }
          to   { opacity: 1; }
        }
        .lt-backdrop-enter {
          animation: lt-backdrop-fade 120ms ease-out;
        }
        @keyframes lt-sheet-fade {
          from { opacity: 0; }
          to   { opacity: 1; }
        }
        .lt-sheet-enter {
          animation: lt-sheet-fade 100ms ease-out;
        }
      `}</style>
        </>
    );
